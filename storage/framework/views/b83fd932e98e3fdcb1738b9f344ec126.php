                ...
                <div class="mt-auto pt-4 border-t border-white/5 flex flex-col gap-3">
                    <div class="flex justify-between items-center">
                        <div class="flex gap-2">
                             <?php if(auth()->user()->role === 'admin' && !$event->is_approved): ?>
                                <form method="POST" action="<?php echo e(route('admin.events.approve', $event)); ?>">
                                    <?php echo csrf_field(); ?>
                                    <button type="submit" class="p-2 rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500/20 transition-all border border-green-500/20" title="Approve Event">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7"/></svg>
                                    </button>
                                </form>
                            <?php endif; ?>
                            <a href="<?php echo e(route('admin.events.edit', $event)); ?>" class="p-2 rounded-lg bg-gold-bright/10 text-gold-bright hover:bg-gold-bright/20 transition-all border border-gold-bright/20" title="Edit Event">
                             ...
                             <?php if(auth()->user()->role === 'admin'): ?>
                                <form method="POST" action="<?php echo e(route('admin.events.destroy', $event)); ?>" onsubmit="return confirm('Delete this event permanently?')">
                                    ...
                                    <button type="submit" class="p-2 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all border border-red-500/20" title="Delete Event">
                        ...
                        <div class="flex items-center gap-2 text-[10px] opacity-40">
                            ...
                            <?php echo e($event->submittedBy->name ?? 'System'); ?>
