...
<div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
    <div>
        ...
        <p class="text-sm mt-1" style="color:var(--gold-muted)"><?php echo e($sermons->total()); ?> recorded messages in the library</p>
    </div>
    <div class="flex gap-3">
        <form method="GET" action="<?php echo e(route('admin.sermons.index')); ?>" class="search-wrap" style="width:300px">
                        ...
                        <div class="flex gap-2">
                             <?php if(!$sermon->is_approved): ?>
                                <form method="POST" action="<?php echo e(route('admin.sermons.approve', $sermon->id)); ?>">
                                    <?php echo csrf_field(); ?>
                                    <button type="submit" class="p-2 rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500/20 transition-all border border-green-500/20" title="Approve Sermon">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7"/></svg>
                                    </button>
                                </form>
                            <?php endif; ?>
                            <a href="<?php echo e(route('sermons.show', $sermon->id)); ?>" class="p-2 rounded-lg bg-gold-bright/10 text-gold-bright hover:bg-gold-bright/20 transition-all border border-gold-bright/20" title="Preview Sermon">
                             ...
                             <?php if(auth()->user()->role === 'admin'): ?>
                                <form method="POST" action="<?php echo e(route('admin.sermons.destroy', $sermon->id)); ?>" onsubmit="return confirm('Delete this sermon permanently?')">
                                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                    <button type="submit" class="p-2 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all border border-red-500/20" title="Delete Sermon">
