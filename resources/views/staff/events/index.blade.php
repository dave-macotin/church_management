...
@section('title', 'Events — Grace Church Staff')

@section('content')
<div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
    <div>
        <h1 class="font-cinzel text-2xl" style="color:var(--gold-mid)">Ministry Events</h1>
        <p class="text-sm mt-1" style="color:var(--gold-muted)">{{ $events->total() }} events cataloged</p>
    </div>
    <a href="{{ route('staff.events.create') }}" class="btn btn-gold">
...
<form method="GET" class="flex flex-col md:flex-row gap-4 mb-8">
    ...
    <div class="flex gap-2">
        ...
        @if(request('search') || request('filter'))
            <a href="{{ route('staff.events.index') }}" class="btn btn-ghost">Clear</a>
        @endif
    </div>
</form>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    @forelse($events as $event)
        <div class="card overflow-hidden flex flex-col group transition-all duration-300 hover:-translate-y-1" style="{{ !$event->is_approved ? 'border-color: rgba(250,199,117,0.3);' : '' }}">
            <div class="aspect-video bg-black/40 relative">
                @if($event->image)
                    <img src="{{ asset('storage/' . $event->image) }}" class="w-full h-full object-cover {{ !$event->is_approved ? 'opacity-40 grayscale' : 'opacity-80 group-hover:opacity-100' }} transition-all" alt="Event">
                @else
                ...
                <div class="absolute bottom-2 right-2 badge badge-muted text-[10px]" style="background: rgba(0,0,0,0.6);">
                    {{ $event->StartDateTime?->format('M d, Y') }}
                </div>
            </div>

            <div class="p-5 flex-1 flex flex-col">
                <div class="flex justify-between items-start mb-2">
                    <h3 class="font-cinzel text-base" style="color:var(--gold-mid)">{{ $event->Title }}</h3>
                ...
                <div class="space-y-1.5 mb-4 opacity-70">
                    <div class="flex items-center gap-2 text-[11px]" style="color:var(--cream)">
                        ...
                        {{ $event->Location ?? 'Grace Church Main Hall' }}
                    </div>
                    <div class="flex items-center gap-2 text-[11px]" style="color:var(--cream)">
                ...
                <div class="mt-auto pt-4 border-t border-white/5 flex flex-col gap-3">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex gap-2">
                             <a href="{{ route('staff.events.show', $event) }}" class="btn btn-gold flex items-center gap-2 text-[10px] py-1.5 px-3 h-auto">
                            ...
                            <a href="{{ route('staff.events.edit', $event) }}" class="btn btn-gold flex items-center gap-2 text-[10px] py-1.5 px-3 h-auto" style="background:rgba(250,199,117,0.05); color:rgba(250,199,117,0.6); border-color:rgba(250,199,117,0.1);">
