...
@section('title', 'Propose Event — Grace Church Staff')
    ...
    <form method="POST" action="{{ route('staff.events.store') }}" enctype="multipart/form-data">
        ...
        <div class="flex flex-col gap-4">
            <div>
                ...
                <input type="text" name="Title" value="{{ old('Title') }}" class="form-input" placeholder="e.g. Worship Night" required>
            </div>
            <div>
                <label class="form-label text-[11px] uppercase tracking-wider text-gold-muted mb-2 block">Location</label>
                <input type="text" name="Location" value="{{ old('Location') }}" class="form-input" placeholder="e.g. Main Hall">
            </div>
