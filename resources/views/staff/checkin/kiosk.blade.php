...
@section('title', 'Child Check-In Kiosk — Grace Church Staff')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="card p-8 flex flex-col justify-center items-center">
            ...
            <form method="POST" action="{{ route('staff.checkin.process') }}" class="w-full">
                @csrf
                <div class="mb-6">
                ...
                @forelse($recentCheckIns as $log)
                    <div class="flex items-center justify-between p-3 rounded bg-white/5 border border-white/5">
                        <div class="flex items-center gap-3">
                            <div class="w-2 h-2 rounded-full {{ $log->type === 'check-in' ? 'bg-[#5DCAA5]' : 'bg-[#D85A30]' }}"></div>
                            <div>
                        ...
                        <div class="text-[10px] uppercase font-bold {{ $log->type === 'check-in' ? 'text-[#5DCAA5]' : 'text-[#D85A30]' }}">
                            {{ $log->type }}
                        </div>
                    </div>
                @empty
