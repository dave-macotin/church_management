...
@section('title', 'Sermons — Grace Church Staff')
...
<div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
    ...
    <div class="flex flex-col md:flex-row gap-3">
        <form method="GET" action="{{ route('staff.sermons.index') }}" class="search-wrap" style="width:300px">
...
<div id="submitForApprovalForm" class="card p-6 max-w-2xl mb-12" style="border: 1px solid var(--gold-bright); background: linear-gradient(135deg, var(--bg-card) 0%, #2a1a0a 100%);">
    ...
    <form method="POST" action="{{ route('staff.sermons.store') }}" enctype="multipart/form-data">
        @csrf
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    @forelse($sermons as $sermon)
        <div class="card overflow-hidden flex flex-col group transition-all duration-300 hover:-translate-y-1" style="{{ !$sermon->is_approved ? 'border-color: rgba(250,199,117,0.3);' : '' }}">
            ...
            <div class="p-5 flex-1 flex flex-col">
                ...
                <h3 class="font-cinzel text-base mb-2" style="color:var(--gold-mid)">{{ $sermon->title }}</h3>
