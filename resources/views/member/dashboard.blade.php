...
@section('content')
    ...
    <div class="welcome-banner" style="background:linear-gradient(135deg, var(--bg-card) 0%, var(--bg-panel) 100%); border:1px solid var(--border); border-radius:16px; padding:32px; display:flex; align-items:center; gap:24px; margin-bottom:32px; position:relative; overflow:hidden;">
        ...
        <div class="welcome-avatar" style="width:80px; height:80px; border-radius:50%; border:3px solid var(--gold-bright); {{ auth()->user()->member?->profile_picture ? 'background-image:url('.asset('storage/'.auth()->user()->member->profile_picture).'); background-size:cover; background-position:center; color:transparent;' : 'background:var(--bg-hover); display:flex; align-items:center; justify-content:center; font-size:32px; font-weight:700; color:var(--gold-bright); font-family:Outfit;' }}">
            {{ !auth()->user()->member?->profile_picture ? strtoupper(substr(auth()->user()->first_name ?? 'M', 0, 1)) : '' }}
        </div>
        <div class="welcome-text" style="flex:1;">
            ...
            <div class="welcome-name" style="font-size:32px; font-weight:700; color:var(--cream); line-height:1.2;">{{ auth()->user()->first_name ?? 'Beloved Member' }}</div>
            <div class="welcome-sub" style="font-size:13px; color:var(--gold-muted); margin-top:6px;">Faithful member since {{ auth()->user()->member?->created_at?->format('F Y') ?? 'this blessed year' }}</div>
        </div>
    ...
    <div class="stats-grid" style="display:grid; grid-template-columns:repeat(4, 1fr); gap:20px; margin-bottom:32px;">
        <div class="stat-card" style="background:linear-gradient(145deg, var(--bg-card) 0%, #1a0f05 100%); border:1px solid var(--border); border-radius:20px; padding:24px; transition:all 0.3s ease; cursor:default; box-shadow:0 10px 30px rgba(0,0,0,0.15);" onmouseover="this.style.transform='translateY(-8px)'; this.style.borderColor='var(--gold-bright)'" onmouseout="this.style.transform='none'; this.style.borderColor='var(--border)'">
            <div class="stat-header" style="display:flex; justify-content:space-between; margin-bottom:16px;">
                ...
                <span class="stat-trend {{ $attendanceRate >= 80 ? 'up' : 'down' }}" style="color:{{ $attendanceRate >= 80 ? 'var(--green)' : 'var(--red)' }}; font-size:11px; font-weight:700; background:{{ $attendanceRate >= 80 ? 'var(--green-bg)' : 'var(--red-bg)' }}; padding:2px 8px; border-radius:20px;">{{ $attendanceRate >= 80 ? '↑ Robust' : '↓ Low' }}</span>
            </div>
            <div class="stat-value" style="font-size:38px; font-weight:800; color:var(--cream); font-family:Outfit; letter-spacing:-0.02em;">{{ $attendanceRate }}<span style="font-size:16px; color:var(--gold-muted); font-weight:500; margin-left:2px;">%</span></div>
        ...
        <div class="stat-card" style="background:linear-gradient(145deg, var(--bg-card) 0%, #1a0f05 100%); border:1px solid var(--border); border-radius:20px; padding:24px; transition:all 0.3s ease; cursor:default; box-shadow:0 10px 30px rgba(0,0,0,0.15);" onmouseover="this.style.transform='translateY(-8px)'; this.style.borderColor='var(--blue)'" onmouseout="this.style.transform='none'; this.style.borderColor='var(--border)'">
            ...
            <div class="stat-value" style="font-size:38px; font-weight:800; color:var(--cream); font-family:Outfit; letter-spacing:-0.02em;">{{ $upcomingCount }}</div>
        ...
        <div class="stat-card" style="background:linear-gradient(145deg, var(--bg-card) 0%, #1a0f05 100%); border:1px solid var(--border); border-radius:20px; padding:24px; transition:all 0.3s ease; cursor:default; box-shadow:0 10px 30px rgba(0,0,0,0.15);" onmouseover="this.style.transform='translateY(-8px)'; this.style.borderColor='var(--purple)'" onmouseout="this.style.transform='none'; this.style.borderColor='var(--border)'">
            ...
            <div class="stat-value" style="font-size:38px; font-weight:800; color:var(--cream); font-family:Outfit; letter-spacing:-0.02em;">{{ $groupCount }}</div>
        ...
        <div class="stat-card" style="background:linear-gradient(145deg, var(--bg-card) 0%, #1a0f05 100%); border:1px solid var(--border); border-radius:20px; padding:24px; transition:all 0.3s ease; cursor:default; box-shadow:0 10px 30px rgba(0,0,0,0.15);" onmouseover="this.style.transform='translateY(-8px)'; this.style.borderColor='var(--green)'" onmouseout="this.style.transform='none'; this.style.borderColor='var(--border)'">
            ...
            <div class="stat-value" style="font-size:38px; font-weight:800; color:var(--cream); font-family:Outfit; letter-spacing:-0.02em;">₱{{ number_format($totalDonations, 0) }}</div>
        ...
        <div style="display:flex; flex-direction:column; gap:24px;">
            ...
            <div class="panel" style="background:var(--bg-card); border:1px solid var(--border); border-radius:20px; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,0.15);">
                <div class="panel-header" style="padding:24px; border-bottom:1px solid var(--border-soft); display:flex; justify-content:space-between; align-items:center; background:linear-gradient(90deg, rgba(239,159,39,0.05) 0%, transparent 100%);">
                    ...
                    <a href="{{ route('member.events') }}" class="panel-action" style="color:var(--gold-bright); font-size:12px; font-weight:600; text-decoration:none; text-transform:uppercase; letter-spacing:0.05em;">View all history</a>
                </div>
                    ...
                    @forelse($upcomingEvents as $event)
                        <div class="event-item" style="padding:16px 24px; display:flex; align-items:center; gap:16px; border-bottom:1px solid var(--border-soft); transition:background 0.2s;" onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background='transparent'">
                            <div class="event-date-badge" style="width:50px; height:50px; background:var(--bg-input); border:1px solid var(--border); border-radius:12px; display:flex; flex-direction:column; align-items:center; justify-content:center; flex-shrink:0;">
                                <div class="day" style="font-size:18px; font-weight:700; color:var(--gold-bright); font-family:Outfit; line-height:1;">{{ $event->StartDateTime->format('d') }}</div>
                                <div class="mon" style="font-size:10px; font-weight:600; color:var(--gold-muted); text-transform:uppercase; margin-top:2px;">{{ $event->StartDateTime->format('M') }}</div>
                            </div>
                            <div class="event-info" style="flex:1;">
                                <div class="event-title" style="font-size:15px; font-weight:600; color:var(--cream); margin-bottom:4px; font-family:Outfit;">{{ $event->Title }}</div>
                                <div class="event-meta" style="display:flex; align-items:center; gap:12px; font-size:12px; color:var(--gold-muted);">
                                    <span style="display:flex; align-items:center; gap:4px;"><svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2"/></svg> {{ $event->StartDateTime->format('g:i A') }}</span>
                                    <span style="display:flex; align-items:center; gap:4px;"><svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" stroke-width="2"/></svg> {{ $event->Location ?? 'Sanctuary' }}</span>
                                </div>
                            </div>
                            <span class="event-chip {{ $event->StartDateTime->isToday() ? 'today' : 'upcoming' }}" style="background:{{ $event->StartDateTime->isToday() ? 'var(--blue-bg)' : 'var(--bg-input)' }}; color:{{ $event->StartDateTime->isToday() ? 'var(--blue)' : 'var(--gold-muted)' }}; padding:4px 10px; border-radius:6px; font-size:11px; font-weight:600; border:1px solid {{ $event->StartDateTime->isToday() ? 'rgba(123,184,232,0.2)' : 'var(--border)' }};">
                                {{ $event->StartDateTime->isToday() ? 'Today' : $event->StartDateTime->diffForHumans() }}
                            </span>
                        </div>
                    @empty
            ...
            <div class="panel" style="background:var(--bg-card); border:1px solid var(--border); border-radius:16px; overflow:hidden; box-shadow:var(--shadow);">
                <div class="panel-header" style="padding:20px 24px; border-bottom:1px solid var(--border-soft); display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <a href="{{ route('member.attendance') }}" class="panel-action" style="color:var(--gold-bright); font-size:12px; font-weight:600; text-decoration:none; text-transform:uppercase; letter-spacing:0.05em;">View full report</a>
                </div>
                <div class="panel-body" style="padding:8px 0;">
                    @forelse($recentAttendance as $record)
                        <div class="attendance-row" style="padding:12px 24px; display:flex; align-items:center; gap:16px; border-bottom:1px solid var(--border-soft); transition:all 0.2s;" onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background='transparent'">
                            <div class="attendance-status-indicator" style="width:10px; height:10px; border-radius:50%; background:{{ $record->Status === 'Present' ? 'var(--green)' : 'var(--red)' }}; box-shadow:0 0 10px {{ $record->Status === 'Present' ? 'rgba(93,202,165,0.4)' : 'rgba(240,153,123,0.4)' }}; flex-shrink:0;"></div>
                            <div class="attendance-info" style="flex:1;">
                                <div class="attendance-event" style="font-size:14px; font-weight:600; color:var(--cream); font-family:Outfit;">{{ $record->event ? $record->event->Title : 'Sacred Sunday Service' }}</div>
                                <div class="attendance-date" style="font-size:12px; color:var(--gold-muted);">{{ $record->Timestamp->format('M d, Y') }}</div>
                            </div>
                            <span class="attendance-status-label" style="font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; color:{{ $record->Status === 'Present' ? 'var(--green)' : 'var(--red)' }}; opacity:0.8;">{{ $record->Status }}</span>
                        </div>
                    @empty
                    ...
                    <div class="rate-container">
                        <div class="rate-circle">
                            <svg viewBox="0 0 36 36">
                                ...
                                <circle class="rate-fill" cx="18" cy="18" r="14" stroke-dasharray="87.96" stroke-dashoffset="{{ 87.96 - (87.96 * $attendanceRate / 100) }}"/>
                            </svg>
                        ...
                        <div class="rate-breakdown">
                            ...
                            <div class="rate-item e">
                                ...
                                <div class="val">{{ $excusedCount }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="panel-body" style="padding:16px 24px;">
                    @forelse($myGroups as $group)
                        <div class="group-item" style="display:flex; align-items:center; gap:16px; margin-bottom:16px; last-child:margin-bottom:0;">
                            ...
                            <div style="flex:1;">
                                <div class="group-name" style="font-size:15px; font-weight:600; color:var(--cream); font-family:Outfit;">{{ $group->GroupName }}</div>
                                <div class="group-desc" style="font-size:12px; color:var(--gold-muted); margin-top:2px;">{{ Str::limit($group->Description, 60) }}</div>
                            </div>
                        </div>
                    @empty
