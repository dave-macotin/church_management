...
<div class="max-w-4xl mx-auto">
    ...
    <div class="card p-8 mb-10 bg-gradient-to-br from-gold-bright/10 via-black to-black border-gold-bright/20">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                ...
                <h2 class="font-cinzel text-3xl text-gold-mid mb-3">The {{ $family->FamilyName }} Family</h2>
                <div class="flex flex-wrap items-center gap-4 text-sm text-gold-muted/80">
                    <span class="flex items-center gap-2">
                        ...
                        {{ $family->HomeAddress ?? 'No address recorded' }}
                    </span>
            ...
            <div class="bg-gold-bright/5 p-4 rounded-2xl border border-gold-bright/20 text-center min-w-[100px] backdrop-blur-sm">
                <div class="font-cinzel text-3xl font-bold text-gold-bright leading-none">{{ $familyMembers->count() }}</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="card p-5 group flex items-center gap-5 transition-all duration-300 hover:border-gold-bright/30">
            <div class="user-avatar btn-sm" style="{{ $member->profile_picture ? 'background-image:url('.asset('storage/'.$member->profile_picture).');' : '' }}">
                @if(!$member->profile_picture)
                    {{ strtoupper(substr($member->FirstName, 0, 1)) }}
                @endif
            </div>
            <div class="flex-1">
                <div class="font-semibold text-cream group-hover:text-gold-bright transition-colors">{{ $member->FirstName }} {{ $member->LastName }}</div>
                <div class="text-[11px] text-gold-muted mt-1 uppercase tracking-wider font-bold">
                    {{ $member->role->RoleName ?? 'Member' }}
                </div>
            </div>
            <div class="text-right">
                <span class="badge {{ $member->Status === 'Active' ? 'badge-green' : 'badge-red' }}">{{ $member->Status }}</span>
            </div>
        </div>
        @endforeach
    </div>
