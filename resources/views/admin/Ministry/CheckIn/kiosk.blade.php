...
@section('title', 'Child Check-In Kiosk — ' . App\Models\Setting::get('church_name', 'Grace Church'))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="card p-8 flex flex-col justify-center items-center">
            ...
            <form method="POST" action="{{ route('admin.checkin.process') }}" class="w-full">
                @csrf
                <div class="mb-6">
                ...
                @forelse($recentCheckIns as $log)
                    <div class="flex items-center justify-between p-3 rounded bg-white/5 border border-white/5">
                        <div class="flex items-center gap-3">
                            <div class="w-2 h-2 rounded-full {{ $log->type === 'check-in' ? 'bg-green-500' : 'bg-red-500' }}"></div>
                            <div>
                                <div class="text-sm font-bold">{{ $log->child->FirstName }} {{ $log->child->LastName }}</div>
                                <div class="text-[10px]" style="color:var(--text-muted)">{{ $log->timestamp->diffForHumans() }}</div>
                            </div>
                        </div>
                        <div class="text-[10px] uppercase font-bold {{ $log->type === 'check-in' ? 'text-green-500' : 'text-red-500' }}">
                            {{ $log->type }}
                        </div>
                    </div>
                @empty
