...
@section('title', 'System Users — Grace Church CMS')
...
{{-- Filters --}}
<form method="GET" action="{{ route('admin.users.index') }}" class="flex gap-3 mb-5 flex-wrap">
            ...
            <tbody>
                @forelse($users as $user)
                    <tr>
                        <td>
                            <div class="flex items-center gap-3">
                                <div class="user-avatar" 
                                     style="{{ $user->member?->profile_picture ? 'background-image:url('.asset('storage/'.$user->member->profile_picture).'); color:transparent;' : 'background:var(--bg-hover);color:var(--gold-mid);' }}">
                        ...
                        <td style="color:var(--gold-muted)">{{ $user->email }}</td>
                        <td>
                            @if($user->role === 'admin')
