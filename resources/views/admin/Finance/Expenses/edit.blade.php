...
@section('title', 'Edit Expense — Grace Church CMS')
    ...
    <form method="POST" action="{{ route('admin.expenses.update', $expense) }}">
        ...
        <div class="flex flex-col gap-4">
            <div>
                ...
                <input type="text" name="Category" value="{{ old('Category', $expense->Category) }}" class="form-input">
            </div>
            <div>
                <label class="form-label">Amount <span style="color:var(--red)">*</span></label>
                <input type="number" step="0.01" name="Amount" value="{{ old('Amount', $expense->Amount) }}" class="form-input" required min="0">
            </div>
            <div>
                ...
                <select name="EventID" class="form-input">
                    ...
                    @foreach($events as $event)
                        <option value="{{ $event->EventID }}"
                            {{ old('EventID', $expense->EventID) == $event->EventID ? 'selected' : '' }}>
